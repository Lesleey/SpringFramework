...
public abstract class TransactionAspectSupport implements BeanFactoryAware, InitializingBean {
	...
	protected Object invokeWithinTransaction(Method method, @Nullable Class<?> targetClass,
		...
		if (txAttr == null || !(ptm instanceof CallbackPreferringPlatformTransactionManager)) {
			...
			//4.1 根据事务管理器、事务属性和 连接点唯一标识符创建 事务信息对象
	 ...
	 *
	 *  根据被给的事务属性决定将要使用的事务管理器
	 */
	@Nullable
	protected TransactionManager determineTransactionManager(@Nullable TransactionAttribute txAttr) {
		// Do not attempt to lookup tx manager if no tx attributes are set
		//1. 没有设置事务属性，则返回默认的事务管理器（可以为 null）
		if (txAttr == null || this.beanFactory == null) {
			return getTransactionManager();
		}
		//2. 如果事务属性指定了 事务管理器的 beanName 则使用该名称获取
		String qualifier = txAttr.getQualifier();
		if (StringUtils.hasText(qualifier)) {
			return determineQualifiedTransactionManager(this.beanFactory, qualifier);
		}
		//3. 如果指定了全局的事务管理器的 beanName 则使用该名称获取
		else if (StringUtils.hasText(this.transactionManagerBeanName)) {
			return determineQualifiedTransactionManager(this.beanFactory, this.transactionManagerBeanName);
		}
		//4. 否则使用默认名称获取 - 使用 类型获取
		else {
	...
	protected TransactionInfo createTransactionIfNecessary(@Nullable PlatformTransactionManager tm,
		...
		//1. 如果事务属性没有名称，则使用 连接点的唯一标识作为该事务属性的名称
	 ...
	 *
	 *   根据被给的事务属性和事务状态对象构建事务信息对象, 并绑定到当前线程中
	 */
