 ...
 *
 *   实现了 spring 标准的事务流程的抽象的基本类，该类提供了以下处理流程
 *     1. 决定是否存在与现有事务中
 *     2. 应用合适的传播级别
 *     3. 如果必须的话挂起或者恢复事务
 *     4. 提交时检查是否仅回滚
 *     5. 在回滚时应用合适的修改
 *     6. 触发注册同步回调（如果事务同步是活跃的）
 *     子类必须提供特定模板方法的实现，比如开始、挂起、恢复、提交、回滚等等。
 *     事务同步是一个通用的机制注册回调，该回调会在事务完成时调用。
 */
...
public abstract class AbstractPlatformTransactionManager implements PlatformTransactionManager, Serializable {
	 ...
	 * @see #doBegin
	 *
	 *    处理事务的传播级别，会委托向 doGetTransaction、isExistingTransaction 、 doBegin
	 */
	@Override
	public final TransactionStatus getTransaction(@Nullable TransactionDefinition definition)
		...
		//3. 如果当前不存在事务，且当前方法的定义的传播级别为 PROPAGATION_MANDATORY 则抛出异常
		if (def.getPropagationBehavior() == TransactionDefinition.PROPAGATION_MANDATORY) {
			throw new IllegalTransactionStateException(
					"No existing transaction found for transaction marked with propagation 'mandatory'");
		}
		//4. 如果当前不存在事务，但是必须需要事务，则开始新的事务
		...
		//5.  创建一个空的事务，并在适当的情况下将事务状态同步到线程中
		else {
	 ...
	 *
	 *   外部事务已存在的环境中，根据事务属性创建事务状态
	 */
	private TransactionStatus handleExistingTransaction(
			TransactionDefinition definition, Object transaction, boolean debugEnabled)
			throws TransactionException {

		//1. 如果 事务传播级别为 PROPAGATION_NEVER 则抛出异常
		if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_NEVER) {
			throw new IllegalTransactionStateException(
					"Existing transaction found for transaction marked with propagation 'never'");
		}

		//2. 如果传播级别为 PROPAGATION_NOT_SUPPORTED
		...
		//3. 如果传播级别为 PROPAGATION_REQUIRES_NEW
		...
		if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_NESTED) {
			...
			//4.2 如果无法使用提交点，则开始一个新的事务（通常仅仅使用于 JTA）
			else {
	 ...
	 *
	 *   根据被给的参数创建 新的事务状态。而且在适当的情况初始事务同步工作
	 */
	...
	private void cleanupAfterCompletion(DefaultTransactionStatus status) {
		...
		//2. 如果为新的事务，则从当前线程中清除事务相关的信息
		if (status.isNewTransaction()) {
			doCleanupAfterCompletion(status.getTransaction());
		}
		//3. 如果存在挂起的事务，则恢复挂起的事务
