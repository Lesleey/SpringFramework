...
class CglibAopProxy implements AopProxy, Serializable {
	...
	public Object getProxy(@Nullable ClassLoader classLoader) {
		...
		try {
			...
			//1.  如果当前的 目标类本身就为 cglib 的代理类（使用 cglib代理生成的类的类名称包含 两个 $$ 符号）则获取原始类
			...
			//5. 设置回调过滤器（根据方法对象获取应该执行回调在回调数组中的索引）
	...
	// 根据目标类和相应的代理配置获取创建代理所需的所有回调（方法拦截器）
	private Callback[] getCallbacks(Class<?> rootClass) throws Exception {
		...
		//1. 根据 aop配置获取对应的拦截器链(增强相关的配置)
		Callback aopInterceptor = new DynamicAdvisedInterceptor(this.advised);

		// Choose a "straight to target" interceptor. (used for calls that are
		// unadvised but can return this). May be required to expose the proxy.
		//2. 对于没有增强的方法，则直接使用 targetInterceptor 对目标对象进行方法调用
		...
		//3. 如果为静态且配置已经为冻结，则为每个方法获取一个固定拦截器连
	 ...
	 * proxy is not frozen.
	 *   当代理没有被冻结或者目标是动态时，该拦截器将会作为通用的 aop 回调
	 */
	private static class DynamicAdvisedInterceptor implements MethodInterceptor, Serializable {
		...
		public Object intercept(Object proxy, Method method, Object[] args, MethodProxy methodProxy) throws Throwable {
			...
			try {
				...
				// 3. 根据目标类和方法对象获取对应的拦截器链
				List<Object> chain = this.advised.getInterceptorsAndDynamicInterceptionAdvice(method, targetClass);
				Object retVal;
				// Check whether we only have one InvokerInterceptor: that is,
				// no real advice, but just reflective invocation of the target.
				//4. 如果拦截器链为空，则表示没有增强器，通过反射调用目标方法
				...
				//5. 如果存在拦截器链，则根据拦截器链进行调用
				else {
					// We need to create a method invocation...
					retVal = new CglibMethodInvocation(proxy, target, method, args, targetClass, chain, methodProxy).proceed();
				}
				//6. 处理返回类型，如果方法返回值为目标类，则返回对应的代理类
	...
	private static class ProxyCallbackFilter implements CallbackFilter {
		 ...
		 *
		 *   该方法的返回值为我们需要的 回调的索引
		 *   1. 对于暴露代理来说，需要代码在 执行调用链前/后对暴露代理的处理，意味这我们需要 使用DynamicAdvisedInterceptor, 而不用在所有的其他
		 *   拦截器中使用 try/catch 块
		 *   2. 对于 finalize() 方法来说，将会使用 SerializableNoOp，没有覆盖的方法去使用
		 *   3.	对于 equals() 方法来说，将会使用 EqualsInterceptor，重定向该方法到 对代理的特殊处理
		 *   4. 对于 Advised 类中的方法来说，将会直接分发调用到目标类中
		 *   5. 对于被增强的方法来说，如果目标原始静态的且拦截器链已经被冻结，则使用固定的 FixedChainStaticTargetInterceptor 处理拦截器链的调用
		 *   否则将会使用 DynamicAdvisedInterceptor。
		 *   6. 对于没有被增强的方法，则根据是否为动态和静态，是否暴露代理 使用不同的分发器和拦截器 StaticDispatcher DynamicUnadvisedInterceptor StaticUnadvisedInterceptor
		 *
		 */
		@Override
