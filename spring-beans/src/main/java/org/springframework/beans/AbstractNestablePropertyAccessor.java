 ...
 *
 * 	   该类为所有传统使用案例提供了必须的基础设施，包括集合和数组的处理、嵌套路径属性的处理等等
 */
public abstract class AbstractNestablePropertyAccessor extends AbstractPropertyAccessor {
	 ...
	 *
	 *   递归的处理所有嵌套的属性路径 比如 colleague[a].friends[b].name   先后处理 colleague、friends、name
	 */
	protected AbstractNestablePropertyAccessor getPropertyAccessorForPropertyPath(String propertyPath) {
		int pos = PropertyAccessorUtils.getFirstNestedPropertySeparatorIndex(propertyPath);
		// Handle nested properties recursively.
		// 如果存在 ‘.’说明依然存在嵌套路径，进行递归处理
		if (pos > -1) {
			String nestedProperty = propertyPath.substring(0, pos);
			String nestedPath = propertyPath.substring(pos + 1);
			// 获取 '.' 符号之前的属性名称，根据名称获取属性访问器，在递归处理子路径
	...
	/*
	* 	获取 startIndex 索引之后， 有效的 [] 字符串 的最后一个字符的索引
	* */
	...
	protected static class PropertyTokenHolder {
		...
		//该属性名称的所有连接取值的 key， 例如 people[0][name][length]  keys = [0, name, length] 表示数组 people索引为一的人的姓名的长度
		@Nullable
		public String[] keys;
	}
