...
public class DispatcherServlet extends FrameworkServlet {
	...
	private void initHandlerMappings(ApplicationContext context) {
		...
		//1. 如果检测所有的 HandlerMapping 类，则从 context 中获取所有 HandlerMapping类型及其子类型的类
		...
		//2. 从上下文中获取类型为 HandlerMapping 的类
		else {
			try {
		...
		//3.  确信我们至少注册了一个 处理器映射器，如果没有找到，则获取默认的处理器映射器
	 ...
	 *
	 *   默认的实现是从类路径的 DispatcherServlet.properties 文件中获取使用的 类名（用逗号分割）
	 */
