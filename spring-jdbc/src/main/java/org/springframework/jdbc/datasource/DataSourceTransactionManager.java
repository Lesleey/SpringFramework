...
public class DataSourceTransactionManager extends AbstractPlatformTransactionManager
	...
	/*
	 *  如果当前的事务对象存在 数据库连接持有者，且参与在一个活跃的事务中
	 */
	@Override
	...
	protected void doBegin(Object transaction, TransactionDefinition definition) {
		...
		try {
			//1. 如果 当前事务对象没有 数据库连接持有者  或者 该连接支持有者没有 与事务同步
			...
			//3. 根据 事务属性设置连接的基本属性（获取之前的隔离级别用于在事务完成之后恢复）
	 ...
	 *
	 *  数据源事务对象，代表一个连接持有者， 被数据源事务管理者用作一个事务对象
	 */
	private static class DataSourceTransactionObject extends JdbcTransactionObjectSupport {

		private boolean newConnectionHolder;  // 是否为新的 数据库连接持有者

		private boolean mustRestoreAutoCommit;  // 是否必须为自动提交
