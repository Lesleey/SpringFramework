 ...
 *
 *   该类封装了 Type, 提供了一些简便的方式去获取类的相关信息，包括父类、泛型信息等等
 */
...
public class ResolvableType implements Serializable {
	...
	/**
	 * The underlying Java type being managed. | 被封装的 Type
	 */
	private final Type type;

	/**
	 * Optional provider for the type.  |  可选的 类型提供者
	 */
	@Nullable
	...
	/**
	 * The component type for an array or {@code null} if the type should be deduced.  | 如果当前封装的为数组，则自动检测其组件类型
	 */
	@Nullable
	...
	@Nullable
	private Class<?> resolved;   // 类型对应的 类对象信息

	@Nullable
	private volatile ResolvableType superType;  // 父类

	@Nullable
	private volatile ResolvableType[] interfaces;  // 所有实现的接口

	@Nullable
	private volatile ResolvableType[] generics;  // 该 类型所有的泛型信息
	 ...
	 * @see #getInterfaces()
	 *
	 *   返回 该类作为指定类的 ResolvableType，会搜索所有的父类以及父接口找寻匹配该 指定Type 的类型，如果找不到返回 NONE
	 */
